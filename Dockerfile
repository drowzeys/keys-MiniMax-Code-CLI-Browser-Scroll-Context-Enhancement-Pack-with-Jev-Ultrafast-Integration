...
WORKDIR /src
# MiniMax Code at the exact commit the maintained TUI patches sit on
RUN git init -q . \
...
LABEL org.opencontainers.image.title="MCode Continuous Context, Browser & Local Decision Enhancement Pack" \
      org.opencontainers.image.description="MiniMax Code CLI with continuous 25% context renewal, progress meter, arm64 browser tooling, and local djev-spark/Syntra decision integrations" \
      org.opencontainers.image.source="https://github.com/drowzeys/keys-mcode-continuous-context-browser-decision-enhancement-pack" \
